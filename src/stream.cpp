...
int zmq::stream_t::xsend (msg_t *msg_)
{
    ...
    //  TODO: This is user error, but we must handle this case properly.
    //  We must pretend the rest of the tail is a separate xsend invocation.
    //  This corresponds to the old behavior when using msg_t::more flags.
    zmq_assert (!msg_->is_linked ());
